...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for company in orm['companies.Company'].objects.all().order_by('-id'):
            existing_codes = orm['companies.Company'].objects.filter(code__isnull=False).order_by('-id')
            ...
            while orm['companies.Company'].objects.filter(code=code).exists():
                new_code = new_code + 1
                code = '%s%04d' % (name[:2].upper(), new_code)
            company.code = code
