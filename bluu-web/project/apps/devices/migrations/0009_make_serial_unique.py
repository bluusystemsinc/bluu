...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        dupes = orm['devices.Device'].objects.values('serial').annotate(models.Count('id')).order_by().filter(id__count__gt=1)
        
        suffix = 1
        for device in orm['devices.Device'].objects.filter(serial__in=[item['serial'] for item in dupes]):
            device.serial = device.serial + '_{}'.format(suffix)
            device.save()
            suffix += 1
            print "Warning! Serial number for device with id '{}' has been made unique.".format(device.id)
